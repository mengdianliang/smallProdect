...
//        rotateCube('rotateX(180deg) rotateY(360deg)');
//    },25000);

    setTimeout(function(){
        //console.log(cube.style.transform);
        rotateCube('rotateY(-720deg) rotateX(180deg)');
    },27000);
            ...
            for (j in arr[1]){
                for (k in arr[2]){
                    if(arr[0][i] == -1) {
                        ...
                        uls[m].style.transform = 'translate3d('+160*(arr[0][i])+'px,'+160*(arr[1][j])+'px,'+160*(arr[2][k])+'px)';
                    }
                    m++;
                }
            }
            ...
            for (a in arr[0]){
                for (b in arr[1]){
                    for (c in arr[2]){
                        if(arr[1][b] == 1) {
                            uls[m].style.transition = '1s';
                            uls[m].style.transform = 'translate3d('+160*(arr[0][c])+'px,'+160*(arr[1][b])+'px,'+160*(arr[2][a])+'px)';
                        }
                        m++;
                    }
                }
    ...
    function commons(arr,space,timer1,timer2,timer3){
        var m = 0;

        for (i in arr[0]){
            for (j in arr[1]){
                for (k in arr[2]){
                    if(arr[1][k] == -1 || arr[1][j] == -1 || arr[1][i] == -1) {
                        uls[m].style.transition = '1s';
                        uls[m].style.transform = 'translate3d('+80*(arr[0][k])+'px,'+80*(arr[1][j])+'px,'+80*(arr[2][i])+'px)';
                    }
                    m++;
