 ...
 */
	(function() {
		...
		BezierPlugin._autoCSS = true; //indicates that this plugin can be inserted into the "style" object using the autoCSS feature of TweenLite
 ...
 */
	_gsScope._gsDefine("plugins.CSSPlugin", ["plugins.TweenPlugin","TweenLite"], function(TweenPlugin, TweenLite) {
		...
		var _numExp = /(?:\d|\-\d|\.\d|\-\.\d)+/g,
			...
			/**
			 * @private Returns the style style for a particular property of an element. For example, to get whatever the current "left" style value for an element with an ID of "myElement", you could do:
		...
		var _getFormatter = function(dflt, clr, collapsible, multi) {
			...
			// @private used when other plugins must tween values first, like BezierPlugin or ThrowPropsPlugin, etc. That plugin's setRatio() gets called first so that the values are updated, and then we loop through the MiniPropTweens  which handle copying the values into their appropriate slots so that they can then be applied correctly in the main CSSPlugin setRatio() method. Remember, we typically create a proxy object that has a bunch of uniquely-named properties that we feed to the sub-plugin and it does its magic normally, and then we must interpret those values and apply them to the style because often numbers must get combined/concatenated, suffixes added, etc. to work with style, like boxShadow could have 4 values plus a color.
		...
		/**
		 * @constructor A SpecialProp is basically a style property that needs to be treated in a non-standard way, like if it may contain a complex value like boxShadow:"5px 10px 15px rgb(255, 102, 51)" or if it is associated with another plugin like ThrowPropsPlugin or BezierPlugin. Every SpecialProp is associated with a particular property name like "boxShadow" or "throwProps" or "bezier" and it will intercept those values in the vars object that's passed to the CSSPlugin and handle them accordingly.
			...
			_registerPluginProp = function(p) {
				if (!_specialProps[p]) {
					...
					_registerComplexSpecialProp(p, {parser:function(t, e, p, cssp, pt, plugin, vars) {
						...
						if (!pluginClass) {
							_log("Error: " + pluginName + " exec file not loaded.");
							return pt;
						}
		...
		/**
		 * Registers a special property that should be intercepted from any "style" objects defined in tweens. This allows you to handle them however you want without CSSPlugin doing it for you. The 2nd parameter should be a function that accepts 3 parameters:
		 ...
		 *
		 * TweenLite.to(element, 1, {style:{myCustomProp:100}});
		 *
		 * In the example, of course, we're just changing the width, but you can do anything you want.
		 *
		 * @param {!string} name Property name (or comma-delimited list of property names) that should be intercepted and handled by your function. For example, if I define "myCustomProp", then it would handle that portion of the following tween: TweenLite.to(element, 1, {style:{myCustomProp:100}})
		...
		var _transformProps = ("scaleX,scaleY,scaleZ,x,y,z,skewX,skewY,rotation,rotationX,rotationY,perspective,xPercent,yPercent").split(","),
			 ...
			 * @param {boolean=} parse if true, we'll ignore any _gsTransform values that already exist on the element, and force a reparsing of the style (calculated style)
			...
			_getTransform = _internals.getTransform = function(t, cs, rec, parse) {
				...
				if (m !== _identity2DMatrix) {
					...
					} else if ((!_supports3D || parse || !m.length || tm.x !== m[4] || tm.y !== m[5] || (!tm.rotationX && !tm.rotationY)) && !(tm.x !== undefined && _getStyle(t, "display", cs) === "none")) { //sometimes a 6-element matrix is returned even when we performed 3D transforms, like if rotationX and rotationY are 180. In cases like this, we still need to honor the 3D transforms. If we just rely on the 2D info, it could affect how the data is interpreted, like scaleY might get set to -1 or rotation could get offset by 180 degrees. For example, do a TweenLite.to(element, 1, {style:{rotationX:180, rotationY:180}}) and then later, TweenLite.to(element, 1, {style:{rotationX:0}}) and without this conditional logic in place, it'd jump to a state of being unrotated when the 2nd tween starts. Then again, we need to honor the fact that the user COULD alter the transforms outside of CSSPlugin, like by manually applying new style, so we try to sense that by looking at x and y because if those changed, we know the changes were made outside CSSPlugin and we force a reinterpretation of the matrix values. Also, in Webkit browsers, if the element's "display" is "none", its calculated style value will always return empty, so if we've already recorded the values in the _gsTransform object, we'll just rely on those.
						var k = (m.length >= 6),
							a = k ? m[0] : 1,
							b = m[1] || 0,
							c = m[2] || 0,
		...
		_registerComplexSpecialProp("transform,scale,scaleX,scaleY,scaleZ,x,y,z,rotation,rotationX,rotationY,rotationZ,skewX,skewY,shortRotation,shortRotationX,shortRotationY,shortRotationZ,transformOrigin,svgOrigin,transformPerspective,directionalRotation,parseTransform,force3D,skewType,xPercent,yPercent,smoothOrigin", {parser:function(t, e, p, cssp, pt, plugin, vars) {
			...
			if (m1.svg && (orig || v.svgOrigin)) {
				...
				pt = _addNonTweeningNumericPT(m1, "xOrigin", (originalGSTransform ? m1 : m2).xOrigin, m2.xOrigin, pt, transformOriginString); //note: if there wasn't a transformOrigin defined yet, just start with the destination one; it's wasteful otherwise, and it causes problems with fromTo() tweens. For example, TweenLite.to("#45", 3, {rotation:180, transformOrigin:"50% 50%", delay:1}); TweenLite.fromTo("#45", 3, {scale:0.5, transformOrigin:"50% 50%"}, {scale:1, delay:2}); would cause a jump when the from values revert at the beginning of the 2nd tween.
			...
			if (orig || (_supports3D && has3D && m1.zOrigin)) { //if anything 3D is happening and there's a transformOrigin with a z component that's non-zero, we must ensure that the transformOrigin's z-component is set to 0 so that we can manually do those calculations to get around Safari bugs. Even if the user didn't specifically define a "transformOrigin" in this particular tween (maybe they did it via style directly).
				if (_transformProp) {
					hasChange = true;
		...
		p._onInitTween = function(target, vars, tween) {
			if (!target.nodeType) { //style is only for dom elements
				return false;
			}
			this._target = target;
			this._tween = tween;
			...
			if (vars.className) { //className tweens will combine any differences they find in the style with the vars that are passed in, so {className:"myClass", scale:0.5, left:20} would work.
 ...
 */
	_gsScope._gsDefine("easing.Back", ["easing.Ease"], function(Ease) {
		...
		var w = (_gsScope.GreenSockGlobals || _gsScope),
			...
			_easeReg = Ease.register || function(){}, //put an empty function in place just as a safety measure in case someone loads an OLD version of TweenLite.exec where Ease.register doesn't exist.
...
(function(window, moduleName) {
		...
		var _namespace = function(ns) {
			 ...
			 * inside TweenLite.exec, but if CSSPlugin is loaded first, it should wait to run its code until TweenLite.exec loads and instantiates TweenPlugin
			 ...
			 * </script>
			 * <script src="exec/greensock/v1.7/TweenMax.exec"></script>
			 * <script>
			 *     window.GreenSockGlobals = window._gsQueue = window._gsDefine = null; //reset it back to null (along with the special _gsQueue variable) so that the next load of TweenMax affects the window and we can reference things directly like TweenLite.to(...)
			 * </script>
			 * <script src="exec/greensock/v1.6/TweenMax.exec"></script>
			 * <script>
		...
		var TweenLite = _class("TweenLite", function(target, duration, vars) {
			...
			_autoCSS = function(vars, target) {
				var css = {},
					p;
				for (p in vars) {
					if (!_reservedProps[p] && (!(p in target) || p === "transform" || p === "x" || p === "y" || p === "width" || p === "height" || p === "className" || p === "border") && (!_plugins[p] || (_plugins[p] && _plugins[p]._autoCSS))) { //note: <img> elements contain read-only "x" and "y" properties. We should also prioritize editing style width/height rather than the element's properties.
						css[p] = vars[p];
						delete vars[p];
					}
				}
...
//---- TweenLite instance methods -----------------------------------------------------------------------------
		...
		p._initProps = function(target, propLookup, siblings, overwrittenProps) {
			...
			if (!this.vars.css) if (target.style) if (target !== window && target.nodeType) if (_plugins.css) if (this.vars.autoCSS !== false) { //it's so common to use TweenLite/Max to animate the style of DOM elements, we assume that if the target is a DOM element, that's what is intended (a convenience so that users don't have to wrap things in style:{}, although we still recommend it for a slight performance boost and better specificity). Note: we cannot check "nodeType" on the window inside an iframe.
